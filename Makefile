...
# PROJECT_ROOT is usually '/edx/src/openedx-blockstore-relay/' on an Open edX devstack
...
test: clean
	python -m pytest --ds=cms.envs.test --no-cov --nomigrations $(PROJECT_ROOT)openedx_blockstore_relay/tests/
...
extract_translations: ## extract strings to be translated, outputting .mo files
	rm -rf docs/_build
	cd openedx-blockstore-relay && ../manage.py makemessages -l en -v1 -d django
	cd openedx-blockstore-relay && ../manage.py makemessages -l en -v1 -d djangojs

compile_translations: ## compile translation files, outputting .po files for each supported language
	cd openedx-blockstore-relay && ../manage.py compilemessages

detect_changed_source_translations:
	cd openedx-blockstore-relay && i18n_tool changed
...
dummy_translations: ## generate dummy translation (.po) files
	cd openedx_blockstore_relay && i18n_tool dummy
